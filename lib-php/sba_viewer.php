...
function listTemplates($mode=NULL) {
  ...
  $templatesFolder = str_replace('/lib-php/sba_viewer.php','/templates',$scriptfile);
  $templates = array();
...
function listTemplates_release($releaseThresh='beta',$friendlyNames = FALSE) {
  ...
  if (is_dir($templatesFolder)) {
          ...
          if ($config) {
            ...
            if ($releaseCode>=$minRelease) {
              if ($friendlyNames) {
                ...
                if (!isset($templates[$species])) $templates[$species] = array();
                $templates[$species][$template] = $v;
              } else {
                $templates[] = $template;
              }
            }
      ...
      if ($friendlyNames) {
        ksort($templates);
        foreach ($templates as $species=>&$templ) {
          ksort($templ);
        }
      }
		}
...
function rgbslice($template,$s0,$size,$nocache=FALSE) {
  ...
  $svgfile = str_replace('lib-php','cache/'.$thumbnail_name.'.svg',$scriptfolder);
