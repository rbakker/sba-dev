...
$release = isset($_REQUEST['release']) ? $_REQUEST['release'] : 'alpha';
$f->setChoices(listTemplates_release($release,TRUE),NULL);
...
function formatCitation($df) {
  ...
  if (@$df['doi']) $ans[] = '. [doi <a href="http://dx.doi.org/'.$df['doi'].'">'.htmlspecialchars($df['doi']).'</a>] ';
  if (@$df['url']) $ans[] = '. [<a href="'.$df['url'].'">download</a>] ';
  $ans = implode('',$ans);
  return $ans;
}
...
if (!isset($featuredPlugin)) $featuredPlugin = @$config['autoPlugin'];
$externalPlugin = NULL;
...
if (isset($config['definingCitations'])) {
  foreach ($config['definingCitations'] as $c) {
    $definingCitations[] = formatCitation($c);
  }
  implode("\n",$definingCitations);
}
$otherCitations = array();
if (isset($config['otherCitations'])) {
  foreach ($config['otherCitations'] as $c) {
    $otherCitations[] = formatCitation($c);
  }
  implode("\n",$otherCitations);
}

$about = @file_get_contents('../templates/'.$template.'/creditsLinks.snip');
...
if ($definingCitations) echo '<p/>The defining citations for this atlas template are:<ol><li>'.implode('</li><li>',$definingCitations).'</li></ol>';
if ($otherCitations) echo '<p/>Additional region- or method-specific citations are:<ol><li>'.implode('</li><li>',$otherCitations).'</li></ol>';
