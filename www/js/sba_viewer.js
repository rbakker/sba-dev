...
function sbaViewer_class(config,svgCapable) {
  ...
  // load region tree
  this.regionTree = new regionTree_class(ACR_TO_PARENT,RGB_TO_ACR,BRAIN_REGIONS,ACR_TO_NNID,config.hierarchyOrphanParent);
  ...
  if (!this.hemisphere) {
    this.hemisphere = ''
    var xWidth = rasLimits[0][1]-rasLimits[0][0];
    if (rasLimits[0][0] < -0.25*xWidth) this.hemisphere += 'L'
    if (rasLimits[0][1] > 0.25*xWidth) this.hemisphere += 'R'
    if (this.sliceAspectRatio>=0.9) this.hemisphere = 'LR'
  }

  var relWidthSlice = this.sliceAspectRatio*(this.hemisphere == 'LR' ? 0.5 : 1);
  ...
  // . 3d clipping
  this.clip3d = { signX: 0 };
  if (this.hemisphere === 'LR') this.clip3d.signX = 1;
...
sbaViewer_class.prototype.view3d_innerHTML = function(angle,modeSpacing3d) {
  ...
  var sMin = (this.hemisphere === 'LR' ? 6 : 1);
...
sbaViewer_class.prototype.mouseoverRegion = function(ev,rgb) {
  ...
  var xyPix = tooltip.clientXY(ev,'view2d_svg');
  var xySvg = this.view2d_pix2svg(xyPix);
  var xyCoord = this.svg2mm(xySvg);  
  var x_mm = Math.round(xyCoord[0]*100)/100
  var y_mm = Math.round(100*this.origSlice2mm(this.currentOrigSlice))/100;
  var z_mm = Math.round(xyCoord[1]*100)/100;
  tip += '<span style="font-size: small">RAS&#160;coord.&#160;'+x_mm+',&#160;'+y_mm+',&#160;'+z_mm+'</span>';
...
sbaViewer_class.prototype.regionCenterAndVolume = function(acr) {
  var rgbList = this.getRgbList(acr);
  var ctr, vol=0, vol_left=0, vol_right=0
  var singleHemiphere = true
  if (rgbList.length > 0 && window.RGB_VOLUMES != undefined) {
    var iLargest = 0;
    var vLargest = 0;
    for (var i=0; i<rgbList.length; i++) {
      vol_i = RGB_VOLUMES[rgbList[i]];
      if (typeof vol_i !== 'number') {
        singleHemiphere = false
        vol_left += vol_i.left
        vol_right += vol_i.right
        vol_i = vol_i.both
      }
      vol += vol_i
      if (vol_i>vLargest) {
    ...
    if (singleHemiphere) {
      if (this.hemisphere == 'L') { 
        vol = { both:undefined, left:vol, right:undefined }
        ctr = { both:undefined, left:ctr, right:undefined}
      } else if (this.hemisphere == 'R') {
        vol = { both:vol, left:undefined, right:vol }
        ctr = { both:undefined, left:undefined, right:ctr }
      } else {
        vol = { both:vol, left:undefined, right:undefined }
        ctr = { both:ctr, left:undefined, right:undefined }
      }
    } else {
      vol = { both:vol, left:vol_left, right:vol_right }
    }
  }
...
sbaViewer_class.prototype.view2d_pix2svg = function(xyPix) {
  ...
  pt.x = xyPix[0]; pt.y = xyPix[1]
  var xySvg = pt.matrixTransform(ctm.inverse())
  return [xySvg.x,xySvg.y]
}
