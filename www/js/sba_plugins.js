...
propertiesPlugin_class.prototype.activate = function(sbaViewer,divElem) {
  function ctrText(ctr,hemisphere) {
    if (!ctr) return;
    var os = sbaViewer.nearestOrigSlice(ctr[1]);
    var xz = sbaViewer.mm2svg([ctr[0],ctr[2]]);
    return Math.round(100*ctr[0])/100 +'; '+ Math.round(100*ctr[1])/100 +'; '+ Math.round(100*ctr[2])/100 +
      ' <a class="link" onclick="var os='+os+';markers=[new sbaMarker_class(\''+acr+' ('+hemisphere+')\',os,['+xz[0]+','+xz[1]+'])];sbaViewer.addMarkers(markers,0);sbaViewer.selectOrigSlice(os);">[show]</a>'
  }
  ...
  if (vol) {
    vol = {
      left: Math.round(100*vol.left)/100,
      right: Math.round(100*vol.right)/100,
      total: Math.round(100*vol.both)/100
    }
    attr['Volume [mm3]'] = vol
  } else {
    attr['Volume [mm3]'] = '[unknown]'
  }
  var ctr = ctr_vol[0];
  var label = rgbList.length>1 ? 'Center of largest subregion' : 'Center';
  if (ctr) {
    ctr = {
      left: ctrText(ctr.left,'left'),
      right: ctrText(ctr.right,'right'),
      mean: ctrText(ctr.both,'mean')
    }
    attr[label+' [mm]'] = ctr
  } else {
