  ...
  req.responseHandler = function(ans) {
    ...
    for (var aO in axonalProjections) {
      ...
      for (var k=0; k<apList.length; k++) {
        ...
        var sbaAcr = cocoAcr2sba[cocoAcr] || cocoAcr
        ...
        var cv = sbaViewer.regionCenterAndVolume(sbaAcr);
        if (cv[0]) {
          var ctr = cv[0].left || cv[0].right
          var nearestOrigSlice = sbaViewer.nearestOrigSlice(ctr[1])
          var xz_svg = sbaViewer.mm2svg([ctr[0],ctr[2]])
          var mk = new sbaMarker_class(cocoAcr,nearestOrigSlice,xz_svg);
