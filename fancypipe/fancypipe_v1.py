...
class FancyLog:
  ...
  def jsonifyList(args,summarize=False):
    if summarize and len(args)>32:
      ...
      args[3] = '(...)'
    return [FancyLog.jsonifyValue(v) for v in args]
  ...
  def jsonifyDict(kwargs,summarize=False):
    ...
    if summarize and len(items)>32:
      ...
      items[3] = ('(...)','(...)')
    return { str(k):FancyLog.jsonifyValue(v) for k,v in items }
...
class Task():
  ...
  def getOutput(self,*keys):
    if not global_taskManager:
      raise RuntimeError('No taskManager detected.')
...
class ArgParse:
  ...
  @classmethod
  def parsedArgs(cls,cmdArgs,fancyConfig=None,customArgs=None):
...
class FancyTask(Task,ArgParse):
  ...
  def __init__(self,main=None,logLevel=None,taskManager=None, fancyConfig=None,taskId=None,autoRemove=True,parsedArgs={}):
    if main: 
      # called when decorating a function with @FancyTask
      self.main = main
      self.name = '<{}>'.format(main.__name__)
    ...
    if global_taskManager.runningTask:
      ...
      self._tempdir = op.join(global_taskManager.workDir)
      if fancyConfig:
  ...
  def fromCommandLine(cls,**customArgs):
    ...
    # TaskManager setup.
    parsedArgs = FancyTaskManager.parsedArgs(cmdArgs,customArgs=customArgs)
  ...
  @classmethod
  def runFromCommandLine(cls,saveAs=None,**customArgs):
    return cls.fromCommandLine(**customArgs).run(saveAs=saveAs)
...
class FancyTaskManager(TaskManager,ArgParse):
  ...
  @classmethod
  def runFromCommandLine(cls,TaskToRun,**customArgs):
    TaskToRun.fromCommandLine(**customArgs).run()
