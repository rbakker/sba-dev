...
function parse_svg_atlas($templatePath,$sliceRange,$svgFilePattern,$svgBoundingBox,$bgColors,$crossHairColor) {
  // example usage:
  ...
  // first loop: get all control points to determine the boundingbox in svg space
  $points = array();
  for ($s=$sliceStart; $s!=$sliceEnd+$sliceStep; $s+=$sliceStep) {
    ...
    $points = pointsFromPaths($parsedPaths);
  ...
  for ($s=$sliceStart; $s!=$sliceEnd+$sliceStep; $s+=$sliceStep) {
    ...
    scaleAndStorePaths($svgPaths,$regionsByRgb,$regionsBySlice,$points, $parsedPaths,$s,$sliceIndex,-$bb[0],-$bb[1],$scaleBy);
...
*/
  /*
  file_put_contents($templatePath.'boundingBox.json',json_encode($scaledBoundingBox));

  if (count($label2acr)) {
    file_put_contents($templatePath.'label2acr.json',json_encode($label2acr));
    file_put_contents($templatePath.'label2xy.json',json_encode($label2xy));
  }
  if (count($rgb2acr)) {
    file_put_contents($templatePath.'template/rgb2acr.json',json_encode($rgb2acr));
  }
  if (count($xyLim)) {
    file_put_contents($templatePath.'xylim.json',json_encode($xyLim));
  }
  if (count($slicePos)) {
    file_put_contents($templatePath.'template/slicepos.json',json_encode($slicePos));
  }
  */
