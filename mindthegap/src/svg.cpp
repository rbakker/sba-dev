 ...
 * Both forward and reverse curves are written and the resultant
...
void SVG::writeFinalOutput(std::vector<Region> &rgn, std::string outFileName, pixel bgColor)
{
	...
	//background
    std::string hexBackground = RGBToHex((uint)bgColor.r, (uint)bgColor.g, (uint)bgColor.b);
	ofsFinal << "<rect width=\"" << imageWidth << "\" height=\"" << imageHeight << "\" fill=\"" << hexBackground << "\"  stroke-width=\"0\" /> " << std::endl;
	...
	for(uint i = 0; i < rgn.size(); ++i)
	{
		...
		std::string hexColor = RGBToHex((uint)rgn[i].col.r, (uint)rgn[i].col.g, (uint)rgn[i].col.b);
		if(hexBackground.compare(hexColor) == 0)
			continue;
		
		//RB
		//ofsFinal << "<path ";
		//if(rgn[i].closedPath.size() > 1) ofsFinal << "fill-rule=\"evenodd\" ";
		//ofsFinal << "fill=\"" << hexColor << "\" stroke-width=\"0\" d=\"";
